...
app.get("/v1/students/credits/:credits", (request, response) => {
    const condition = parseInt(request.params.credits);
    const studentsWithCreditsGreaterThan = StudentController.filterByCredits(condition);
    response.json(studentsWithCreditsGreaterThan);
});
